...
What a finding *is* is :class:`~.diagnostic.Diagnostic`; this is the channel by
which the source answers back. Both halves are contract vocabulary, and the
second one arrived here late: while prism's linter was the only reader, the
comment syntax was one linter's mechanism and lived with it. Two linters in two
products read it now, and one of them cannot import the other — so the syntax is
something any linter may read in any tree, which is the definition of this
package.
"""
